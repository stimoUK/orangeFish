...
namespace FishORama
{
    ...
    class AquariumMind : AIPlayer
    {
        ...
        public void CheckRightClickBehavior()
        {
            ...
            if ((Mouse.GetState().RightButton == ButtonState.Pressed) && (mAquarium.Marker == null))
            {

                Vector2 mouseClickPos = new Vector2(Mouse.GetState().X, 560);// so the toxic barrel stops at the bottom of the screen.    
                Vector3 MarkerPos = this.mAquarium.Kernel.Camera.CameraToWorld(mouseClickPos);     

                if ((Math.Abs(MarkerPos.X) < mAquarium.Width / 2) && (Math.Abs(MarkerPos.Y) < mAquarium.Height / 2))
                {
                    
                    mAquarium.Marker = new MarkerToken("Marker1");

                    MarkerPos.Z = 0;// so the marker is not visible on the viewport.
                 
                    this.mAquarium.Kernel.Scene.Place(mAquarium.Marker, MarkerPos);
