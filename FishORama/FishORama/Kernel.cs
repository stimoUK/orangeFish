...
namespace FishORama
{
    ...
    public class Kernel : XNAGame
    {
        ...
        private AquariumToken mAquarium;
        ...
        /*By changing these values it lets 
        me define how many tokens i want.*/
        int numOfPiranha = 1;
        int numOfSeaHorse = 3;
        Random coOrRandom = new Random();
        ...
        protected override AssetLibrary GetAssetLibrary()
        {
            ...
            // Create a new graphic asset  for the toxic fish visuals using class X2DAsset.
            A = new X2DAsset("ToxicBarrelVisuals", "ToxicBarrel").
                UVOriginAt(64, 64).
                UVTopLeftCornerAt(0, 0).
                Width(128).
                Height(128);

            // Import toxic barrel visual asset in the library
            lib.ImportAsset(A);

            // Create a new graphic asset  for the Bubble visuals using class X2DAsset.
            A = new X2DAsset("ToxicBubbleVisuals", "ToxicBubble").
                UVOriginAt(16, 16).
                UVTopLeftCornerAt(0, 0).
                Width(32).
                Height(32);
            ...
            // Create a new graphic asset  for the toxic fish visuals using class X2DAsset.
            A = new X2DAsset("ToxicFishVisuals", "ToxicFish").
                UVOriginAt(64, 64).
                UVTopLeftCornerAt(0, 0).
                Width(128).
                Height(128);

            // Import toxic fish visual asset in the library
            lib.ImportAsset(A);

            // Create a new graphic asset  for the sunken ship visuals using class X2DAsset.
            A = new X2DAsset("SunkenShipVisuals", "SunkenShip").
                UVOriginAt(64, 64).
                UVTopLeftCornerAt(0, 0).
                Width(300).
                Height(300);

            // Import sunken ship visual asset in the library
            lib.ImportAsset(A);
        ...
        protected override void LoadContent()
        {
            ...
            //OrangeFish and ToxicFish
            
            /*if (mAquarium.ToxicBarrel != null)
            {
                Toxic = true;
            }
            if (Toxic == true)
            {              
                List<ToxicFishToken> toxicfish = new List<ToxicFishToken>() { };
                for (int i = 0; i < numOfOrangeFish; i++)
                {
                    toxicfish.Add(new ToxicFishToken("ToxicFish" +i, aquarium));
                }                                               
                foreach (ToxicFishToken token in toxicfish)
                {
                    fishPos = new Vector3(coOrRandom.Next(-400, 400), coOrRandom.Next(-200, 200), 1);
                    mScene.Place(token, fishPos);
                }
            }
            else if (Toxic == false)
            { 
                List<OrangeFishToken> orangeFish = new List<OrangeFishToken>() { };
                for (int i = 0; i < numOfOrangeFish; i++)
                {
                    orangeFish.Add(new OrangeFishToken("OrangeFish" +i, aquarium));
                }                                                  
                foreach (OrangeFishToken token in orangeFish)
                {
                    fishPos = new Vector3(coOrRandom.Next(-400, 400), coOrRandom.Next(-200, 200), 1);
                    mScene.Place(token, fishPos);
                }
            ...
            //orange fish
            Vector3 fishPos;
            //fishPos so i can add attributes to the fish position.
            OrangeFishToken orangeFish = new OrangeFishToken("OrangeFish", aquarium);
            fishPos = new Vector3(coOrRandom.Next(-400, 400), coOrRandom.Next(-200, 200), 1);
            mScene.Place(orangeFish, fishPos);


            //toxic barrel
            Vector3 tBarrelPos;
            ToxicBarrelToken ToxicBarrel = new ToxicBarrelToken("ToxicBarrel", aquarium);
            tBarrelPos = new Vector3(0, 1000, 1);
            mScene.Place(ToxicBarrel, tBarrelPos);
            ...
            for (int i = 0; i < numOfPiranha; i++)
            {
                piranha.Add(new PiranhaToken("Piranha" +i, aquarium));
            }                                          //+i to make so they can do individual things.
            foreach (PiranhaToken token in piranha)
            {
                piranhaPos = new Vector3(coOrRandom.Next(-400, 400), coOrRandom.Next(-200, 200), 1);
                mScene.Place(token, piranhaPos);
            }
            ...
            //Seahorse
            Vector3 seaHorsePos;
            List<SeaHorseToken> seaHorse = new List<SeaHorseToken>() { };

            for (int i = 0; i < numOfSeaHorse; i++)
            {
                seaHorse.Add(new SeaHorseToken("SeaHorse" +i, aquarium));
            }
            foreach (SeaHorseToken token in seaHorse)
            {
                seaHorsePos = new Vector3(coOrRandom.Next(-400, 400), coOrRandom.Next(-200, 200), 1);
                mScene.Place(token, seaHorsePos);
            }
            ...
            //bubbles
            Vector3 bubblePos;
            List<BubbleToken> Bubbles = new List<BubbleToken>();

            for (int i = 0; i < 5; i++)
            {

                Bubbles.Add(new BubbleToken("Bubble" +i, aquarium, orangeFish));

            }
            for (int i = 0; i < Bubbles.Count; i++)
            {        
                    float x = fishPos.X + coOrRandom.Next(0, 100);
                    float y = fishPos.Y + coOrRandom.Next(0, 150);
                    float z = 2;
                    bubblePos = new Vector3(x, y, z);
                    mScene.Place(Bubbles[i], bubblePos);
            }

            //toxic bubbles
            Vector3 toxicBubblePos;
            List<ToxicBubbleToken> toxicBubbles = new List<ToxicBubbleToken>();
            for (int i = 0; i < 5; i++)
            {

                toxicBubbles.Add(new ToxicBubbleToken("ToxicBubble" +i, aquarium, ToxicBarrel));

            }
            for (int i = 0; i < toxicBubbles.Count; i++)
            {
                    float x = tBarrelPos.X + coOrRandom.Next(0, 200);
                    float y = tBarrelPos.Y + coOrRandom.Next(0, 250);
                    float z = 2;
                    toxicBubblePos = new Vector3(x, y, z);
                    mScene.Place(toxicBubbles[i], toxicBubblePos);
            }
        


            //the sunken ship
            SunkenShipToken sunkenship = new SunkenShipToken("SunkenShip", aquarium);
            tokenPos = new Vector3(150, -100, 1);
            mScene.Place(sunkenship, tokenPos);
        ...
        public GameObject piranha1 { get; set; }
    }
}
